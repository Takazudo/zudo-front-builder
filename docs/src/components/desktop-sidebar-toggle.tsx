...
// SPA-navigation guard for the desktop sidebar's hidden-state (zudolab/zudo-doc#2198).
//
...
// the restore would never run. Registering once on `document` at bundle load
// (the same lifecycle-independent pattern as client-router-bootstrap.tsx) keeps
// the guard alive across island unmount/remount. SSR-safe: no-ops when
...
// sidebar/wrapper/toggle transitions). The swap then wipes BOTH the marker and
// data-sidebar-hidden. On AFTER_NAVIGATE_EVENT (swap done → the live <html> is no
// longer wiped) RE-SET the marker, then re-add data-sidebar-hidden in the same
// synchronous batch, so the hidden geometry snaps in with transitions suppressed
// (no slide, no open frame). Remove the marker on a double rAF afterward so a
// later user toggle still animates.
...
export default function DesktopSidebarToggle() {
  ...
  // island's mount/unmount — zfb-runtime unmounts islands before firing
  // AFTER_NAVIGATE_EVENT, so a useEffect listener would be torn down before the
  // swap completes — so it lives at module scope; see
  // registerSidebarSpaNavGuard() above. (zudolab/zudo-doc#2198)
