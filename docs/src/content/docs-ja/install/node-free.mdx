...
Node.js は不要です** — esbuild と Tailwind は `zfb` 実行ファイルと並んでスタンドアロンの
ネイティブバイナリとして（リリース tarball の `binaries/` スロットに）同梱され、`zfb`
実行ファイル自身からの相対位置で見つけられてサブプロセスとして呼び出されます。Node が
必要になるのは一部の機能をオプトインで使う場合だけです（下記の「Node が依然として
必要なケース」を参照）。

Node なしの経路でもコンテンツサイト向けの全機能が利用できます。Markdown・MDX・
HTML ページ、**さらに islands** まで対応します — esbuild と Tailwind は `zfb` に同梱された
スタンドアロンのネイティブバイナリとして動作するため、クライアントサイドの
