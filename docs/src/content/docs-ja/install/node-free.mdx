...
Node.js は不要です** — ビルドエンジン・esbuild・Tailwind はすべてバイナリに
コンパイル済み、あるいは Go バイナリとして同梱されています。Node が必要になるのは
一部の機能をオプトインで使う場合だけです（下記の「Node が依然として必要なケース」を
参照）。

Node なしの経路でもコンテンツサイト向けの全機能が利用できます。Markdown・MDX・
HTML ページ、**さらに islands** まで対応します — esbuild と Tailwind は zfb バイナリ内に
組み込まれた Go バイナリとして動作するため、クライアントサイドのハイドレーションも
Node を一切介さずに動きます。
