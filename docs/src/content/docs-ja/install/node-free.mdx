...
<Note title="1.0 未満の現状：上記のコマンドはまだ何もインストールできません">

現時点では安定版リリースは存在せず、プレリリース（`v0.1.0-next.N`）のみです。
`v0.1.0` がリリースされるまで、デフォルトのコマンドは**安定版のみ**を解決しようと
して、該当するリリースが見つからずに失敗します。プレリリースを明示的にインストール
してください。
