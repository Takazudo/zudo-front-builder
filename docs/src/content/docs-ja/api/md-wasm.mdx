...
`directives: true` の場合（[`ParseToAstOptions`](#parsetoastoptions) を参照。デフォルトは `false`）、ツリーは 3 つの汎用 remark-directive ノード型も持てます。いずれも同じ `DirectiveNodeBase` の形（`position`、`name`、`attributes`、`children`）を共有します:
