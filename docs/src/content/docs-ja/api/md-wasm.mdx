...
#2447 の決定スナップショットにおける正確な最適化後の行（final wasm、gzip-9、glue、glue gzip-9）は次のとおりです:
...
固定された gzip-9 の上限は root 1,600,000 B、highlight 820,000 B、render 1,100,000 B、parse
325,000 B、完全な packed tarball は 3,900,000 B です。クリーンな本番 4 ステップの上限は 210 秒で、
選択されたスナップショットの中央値は 155.015 秒 [153.496, 165.977] です。これらは永続的な保証ではなく、
決定スナップショットの測定値です。

highlight 行は明示的に歴史的なものです。#2447 の現在の `highlight,compiler-surface` ベースラインは、
content エッジが無条件だったため `swc_core` を保持していました。出荷後の #2449 highlight グラフは、
#2450 の完全なパリティと `swc_core` 不在アサーションによって SWC-free だと証明されていますが、公開 API / リソースは
互換のままで、ゲート後のサイズは測定されていません。推測値を測定値として扱わないでください。
