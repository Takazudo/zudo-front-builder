...
Tailwind を有効にすると、ユーティリティクラスは走査対象のコンテンツルート配下にある `.tsx` ファイルで動作します（詳細は下記の [Tailwind が見る場所](#tailwind-が見る場所) を参照）。
...
エイリアス先は「mirror root」になります。CSS パイプラインは、主張された workspace sibling ディレクトリを、上記の [Tailwind が見る場所](#tailwind-が見る場所) で説明した `pages/`・`components/` などのスキャン対象ルートと同じように扱います。
