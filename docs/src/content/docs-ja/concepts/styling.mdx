...
Tailwind まわりは zfb がすべて引き受けます — コンパイラ本体、実行されるバージョン、そして `@import "tailwindcss*"` の解決までです。契約は次の 4 点に尽きます。
...
zfb の CSS import ウォーカーは `tailwindcss` で始まる specifier を認識して解決をまるごとスキップし（`crates/zfb-css/src/css_imports.rs` の `is_virtual_specifier`）、これらのスタイルシートは組み込みバイナリ自身が供給します。そのため `node_modules/tailwindcss` が読まれることはありません。そのまま残しておく構成もサポート対象外です。出力される CSS は変わらず、組み込みのバージョンを上書きすることもなく、lockfile を重くするだけです。
