...
同梱の `basic-blog` テンプレートが、エンドツーエンドの形を実演しています。その `hello-zfb.mdx` は、本文に直接 JSX として `<Tip>` admonition を書いています。
...
投稿ごとのルートには `<Tip>` を配線している箇所はありません。ルートが渡しているのは `defaultComponents` だけです。
...
admonition は、zfb がすべてのエントリにマージするプロジェクトルートの `mdx-components.tsx` から届きます。
...
5 つの名前をルートごとではなくここに置いていることが、**素の `.md`** 記事でもこれらを使えるようにしています。テンプレートは `zfb.config.ts` で `markdown.features.githubAlerts` を有効にしているため、`> [!NOTE]` の引用ブロックはレンダリング前に `<Note>` 要素へ書き換えられます。そしてマップに存在しない PascalCase の名前は、レンダリング時に例外を投げます。ルートごとに配線する形にすると、記事をレンダリングするすべてのルートが 5 つすべてを覚えておかなければなりません。

コンポーネント自体は、ごく普通の Preact（または React）コンポーネントです。`components/callout.tsx` はベースとなるコンポーネント 1 つと、`children` だけを受け取る 5 つの名前付きラッパーをエクスポートします。
...
// …Tip・Important・Warning・Caution も Note と同じく 1 行ずつ。
```

5 つのコンポーネント名は機能側で固定されており、こちらで選べるものではありません。アラートの記法そのものは [GitHub アラート](../markdown-features/github-alerts.mdx)を、これらのファイルがスキャフォールドのどこに置かれるかは [プロジェクト構造](../getting-started/project-structure.mdx) を参照してください。
...
- [Content Collections](./content-collections.mdx) — エントリがどのように発見され、`getCollection()` を通じて公開されるか。
- [Build Pipeline](./build-pipeline.mdx) — MDX から JSX へのコンパイルがエンドツーエンドのフローのどこに位置するか。
- [Custom Directives](./custom-directives.mdx) — 自前の JSX コンポーネントにマッピングされる新しい MDX ディレクティブ構文（例: `:::callout`）を追加する。
- [Islands](./islands.mdx) — コンテンツエントリ内でインタラクティブなコンポーネントを動かす方法。
