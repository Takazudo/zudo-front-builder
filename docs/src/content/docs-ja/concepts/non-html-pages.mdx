...
このルールは `zfb-content::tsx_frontmatter`（`extension` リテラルを抽出）と `zfb-router::route::Route`（ファイル名規約をパース）が共同で適用します。小さなヘルパー `zfb_render::meta::derive_output_extension(frontmatter, route)` が、呼び出し側で優先順位を適用します。
