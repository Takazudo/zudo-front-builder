...
`import.meta.glob` は [クライアントスクリプト](../guides/client-scripts.mdx)（`*.client.{ts,tsx,js,jsx}`、およびそれらがインポートする任意のモジュール）では **サポートされません**。クライアントスクリプトは実際のプロジェクトツリーに対して直接バンドルされ、この経路ではシャドウコピーが走らないため、呼び出しは展開されないままバイト単位でそのままブラウザへ配信されます。上記の島のケースとは異なり、**これはビルド時には検出されません**。`zfb build` は成功し、スクリプトが実際に実行されるときにブラウザが例外を投げます。`import.meta.glob` は Vite の外では `undefined` だからです。クライアントスクリプト内で glob のようなファイル一覧が必要な場合は、以下の代替手段のいずれかを使ってください。
