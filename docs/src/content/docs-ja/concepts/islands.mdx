...
| `"use client"` 島から推移的にインポートされるモジュール | あり（#1404 以降、上記と同じ制約） |
...
以下のいずれか（`"use client"` 島から到達可能なモジュールグラフのどこかに存在するもの）は、問題のファイル名を示すメッセージとともに `zfb build` を失敗させます。`zfb dev` 中は同じメッセージが警告としてログ出力され、そのリバンドルのティックはスキップされるため、ファイルを直して再保存する間も dev サーバーは動き続けます。
