...
`zfb dev` 中、raw ターゲットは import 元とは独立して監視されます。テキストファイルを編集・削除・再作成するとブラウザバンドルが invalidation されます。同じ変換と依存追跡はクライアントスクリプトとページ / SSR パスにも適用されます。
...
ディレクティブは worker が実際にアクセスする対象に合わせて調整してください。`base` を設定している場合は `_headers` のパスにもそのプレフィックスを含めます。[クライアントスクリプト](../guides/client-scripts.mdx#module-worker)が所有する worker は、別の `/assets/client/worker-*.js` ディレクトリに置かれるため、それに対応するルールが必要です。
