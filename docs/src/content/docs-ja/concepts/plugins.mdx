...
- **再実行に失敗したときは直前の正常なソースを保持する。** 再実行した loader が throw した場合（書き込み途中のファイル、保存中に読んでしまった不完全な JSON など）、zfb は直前にメモ化したソースをそのまま保持し、エラーをログに出し、監視を続けます。次に正常にロードできた変更でメモが置き換わるため、ファイルを削除して作り直せば自力で復帰します。
