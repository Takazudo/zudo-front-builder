...
`zfb-server` はまだ crates.io に公開されていないため、`docs.rs/zfb-server` リンクは解決されません。workspace crate に git から依存し、タグまたはコミットを pin してください。
...
`ServerBuilder::with_page_cache(cache)` は、`build()` がデフォルトで割り当てる空のものではなく、あなたが所有する `PageCache` をサーバーに渡します。`PageCache` は `Clone` です（内部的には `Arc<RwLock<…>>`）。そのため、手元にハンドルを保持し、クローンをビルダーに渡してから、実行時に*同じ*共有マップへ書き込めます。稼働中のサーバーはすべての insert を見ます。
...
ページキャッシュ以上のものが必要な場合、たとえば稼働中のライブリロード `broadcast` チャンネル、リクエスト時 SSR ルート（`SsrRoutesHandle`）、プラグイン dev-middleware が必要な場合、ビルダーは意図的に入口ではありません。自分で `ServeOpts` を構築し、低レベルの `serve_with_listener(opts, listener, shutdown)`（またはリスナーもバインドする `serve(opts, shutdown)`）を呼び出します。どちらも公開され安定しています。`zfb dev` 自身が使っているのと同じ入口です。
