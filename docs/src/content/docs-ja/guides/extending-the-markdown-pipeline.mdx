...
`register` 呼び出し以上に Rust へ手を入れる必要はありません。そのパスについては [Custom Directives](/concepts/custom-directives)
を参照してください。
...
| やりたいこと | パス | 置き場所 |
|---|---|---|
| JSX コンポーネントにコンパイルされる `:::name` / `::name` / `:name` 構文を追加する | ディレクティブを登録する | `Pipeline::with_defaults` + `DirectiveRegistry`（[Custom Directives](/concepts/custom-directives)） |
...
昇格のしきい値は [3 消費者ルール](/concepts/design-philosophy)に従います。
...
- [Custom Directives](/concepts/custom-directives) — `:::name` / `::name` / `:name` 構文の作者向けの解説。Rust 不要。
- [Customizing Markdown](/guides/customizing-markdown) — コンテンツコレクション消費者の視点から見た Markdown レンダリングパイプライン。
