...
生成された Worker が実際にどう動くかの概念的なメンタルモデルについては、[SSR on a Worker (adapter mode)](/ja/concepts/ssr-on-a-worker) を参照してください。
...
同じ制約は `frontmatter` エクスポートにも適用されます。リテラルのみのコントラクトについては [Frontmatter](/ja/concepts/frontmatter) を参照してください。
...
このフラグがないと、Worker は `node:async_hooks` を欠落モジュールとして示すエラーとともに起動に失敗します。より深い仕組みについては [SSR on a Worker (adapter mode)](/ja/concepts/ssr-on-a-worker#the-getcloudflarecontext-trick) を参照してください。
