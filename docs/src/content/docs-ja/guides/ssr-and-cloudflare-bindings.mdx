...
- `.assetsignore` — `_worker.js` と `_zfb_inner.mjs` を列挙し、アセットサーバーがサーバーコードを公開ファイルとして配信しないようにします（これがないと `/_worker.js` へのリクエストがそれをプレーンテキストとしてダウンロードさせてしまいます）。
...
<Note title="Cloudflare Pages にも引き続きデプロイ可能">

同じ `dist/` 出力は **Cloudflare Pages のアドバンストモード**にも引き続きデプロイできます。ルート直下の `_worker.js` は Pages における同等の慣習であり、ラッパーは両プラットフォームで同一のディスパッチをします。Workers Static Assets が主たる推奨ターゲットであり、Pages のアドバンストモードは既存の Pages プロジェクトのためにサポートが継続されます。（プラットフォームで見える唯一の違いは末尾スラッシュのアセットリダイレクトで、Workers Static Assets では `307`、Pages では `308` になります。）
