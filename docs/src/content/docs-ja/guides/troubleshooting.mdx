...
- `ZFB_VERSION` が未設定の場合、最新の **非プレリリース** リリースを要求します。1.0 以前の zfb はプレリリース（`v0.1.0-next.N`）しか出していないため、解決すべき安定版リリースがまだ存在せず、これは設計どおり失敗します。[Node なしでインストール](../install/node-free.mdx#linux-macos-—-curl) の注記を参照してください。
...
- 1.0 以前（まだ安定版リリースが存在しない）: 明示的にプレリリースにオプトインする —

  ```sh
  curl -fsSL https://raw.githubusercontent.com/Takazudo/zudo-front-builder/main/install.sh | ZFB_VERSION=latest-prerelease sh
  ```

- `latest-prerelease` 自体が解決に失敗する場合: 代わりに [リリースページ](https://github.com/Takazudo/zudo-front-builder/releases) から厳密なタグを固定してください（例: `ZFB_VERSION=v0.1.0-next.76`）。
