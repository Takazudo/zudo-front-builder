...
TypeScript はこれらのバンドルコントラクトを推論しません。クライアントスクリプトでモード値、カスタム loader の import、raw import、define 名を `zfb check` と `tsc` に通すには、[対応する ambient 宣言](../api/define-config.mdx#typescript-ambient-declarations)を追加してください。
