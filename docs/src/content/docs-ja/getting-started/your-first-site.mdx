...
ホストとポートを示す「ready」バナーが表示されます（デフォルトは `http://localhost:3000`）。zfb はプロジェクトディレクトリを監視します。`pages/`・`layouts/`・`components/`・`content/`・`styles/` 内のファイルを保存すると、接続中のブラウザにライブリロードがブロードキャストされます。サーバーは `public/` に置いたものも配信します。
