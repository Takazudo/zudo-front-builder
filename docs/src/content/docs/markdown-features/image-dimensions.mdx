...
`ImageDimensionsPlugin` runs in the hast phase, **after** `ImageEnlargePlugin` (which wraps block-level images in a `<figure>`). The plugin reads the already-wrapped structure and injects dimensions on the `<img>` inside the figure, if any.
