...
Heading IDs come from [`HeadingLinksPlugin`](/markdown-features/heading-links), which runs earlier in the same hast phase. The cross-file heading-ID registry is populated during the build so anchor validation works across files.
