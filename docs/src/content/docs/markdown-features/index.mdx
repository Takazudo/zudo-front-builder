...
shape includes `boolean`. Object-only rows, such as `codeEnrichment`,
`tocExport`, `imageDimensions`, `linkValidation`, `transclude`, and
`headingIds`, require an object. Unknown keys are rejected at config load
time.
...
|---|---|---|---|---|
| [GFM constructs](./gfm.mdx) | Core | `markdown.gfm` | `boolean \| { strikethrough?: boolean; table?: boolean; autolinkLiteral?: boolean; taskListItem?: boolean; footnoteDefinition?: boolean }` | Absent → conservative default (`strikethrough`/`table`/`autolinkLiteral` on, other two off). `true`/`false` shorthand toggles all five. |
