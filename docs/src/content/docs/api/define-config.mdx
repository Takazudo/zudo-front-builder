...
- `collections?: CollectionDef[]` — content collections. Each entry has:
  ...
  - `schema?: Record<string, unknown>` — optional JSON Schema for frontmatter validation (checked by `zfb check` and the build).
...
- `markdown?: MarkdownConfig` — Markdown/MDX parsing options (e.g. GFM toggle). See the [Markdown Features](/markdown-features) section.
