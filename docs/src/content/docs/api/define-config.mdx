...
- `allowedHosts?: string[]` — Host header values the dev/preview server accepts when bound to a non-localhost interface (mirrors Vite's `server.allowedHosts`). Only consulted for non-loopback binds; `localhost`, `127.0.0.1`, `[::1]`, and the explicitly bound host are always allowed. Entries match exactly (case-insensitive, request port stripped); a leading-dot entry like `".example.com"` also matches subdomains.
