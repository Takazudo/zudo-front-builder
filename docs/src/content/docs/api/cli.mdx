...
| `ZFB_DEV_BOOT_LAZY` | `1` | Opt-in fast boot: when a valid prebuilt `dist/` exists, serve it immediately and defer per-route rendering to the first request instead of rendering every route at boot. Requires lazy rendering (no-op when `ZFB_DEV_EAGER` is set). Falls back to the eager boot render when no servable `dist/` exists. Off by default. |
| `ZFB_DEV_BOOT_LAZY` | `cold` | Seedless boot-lazy: defers per-route rendering to the first request the same way, but without requiring a prebuilt `dist/` at all. Every route serves the dev 404 page (with livereload) until its own first request re-renders it — tabs left open during that window self-heal via livereload once each route resolves. Use when no `dist/` exists yet; `1` would silently fall back to the eager boot render in that case instead. |
...
| `ZFB_DEV_BOOT_LAZY` | unset | Set to `1` to opt in to fast dev-server boot: serve a valid prebuilt `dist/` immediately and defer each route's rendering to its first request (falls back to the eager boot render when no servable `dist/` exists), or set to `cold` for the seedless variant — same deferral, no `dist/` required, every route serves the dev 404 page until its own first request. Requires lazy rendering; no-op when `ZFB_DEV_EAGER` is set. |
