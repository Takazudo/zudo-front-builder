...
- `node-free` — ships no `package.json` and skips the post-scaffold `pnpm install` step entirely, for projects run with no Node/pnpm on `PATH`. See [Install without Node](../install/node-free.mdx).
...
| `ZFB_DEV_BOOT_LAZY` | `1` | Opt-in fast boot: when a valid prebuilt `dist/` exists, serve it immediately and defer per-route rendering to the first request instead of rendering every route at boot. Requires lazy rendering (no-op when `ZFB_DEV_EAGER` is set). Off by default. |
| `ZFB_DEV_DEFER_BUNDLE` | `0` | Opt out of the boot-lazy bundle deferral: build the renderer before bind (no SSR-only 404 window) at the cost of a slower first-accept. On by default when boot-lazy is active. |
...
| `ZFB_DEV_BOOT_LAZY` | unset | Set to `1` to opt in to fast dev-server boot: serve a valid prebuilt `dist/` immediately and defer each route's rendering to its first request. Requires lazy rendering; no-op when `ZFB_DEV_EAGER` is set. |
| `ZFB_DEV_DEFER_BUNDLE` | unset | Set to `0` to opt out of the boot-lazy bundle deferral (build the renderer before bind instead of after the first request). On by default when boot-lazy is active. |
...
| `ZFB_SKIP_WRANGLER_VERSION_CHECK` | unset | Set to `1`, `true`, or `yes` to bypass the `zfb preview` adapter-mode wrangler version gate. Intended for temporary release-engineering use. |
