...
At SSR time the `<Island>` wrapper emits a `<div data-zfb-island="Counter" data-when="visible">` marker with the server-rendered child inside. The generated hydration bootstrap imports `mountIslands` from `"@takazudo/zfb/runtime"`, queries these markers, and mounts the component when the `when` condition fires.
