...
With `directives: true` (see [`ParseToAstOptions`](#parsetoastoptions), default `false`), the tree can also carry the three generic remark-directive node types, all sharing the same `DirectiveNodeBase` shape (`position`, `name`, `attributes`, `children`):
