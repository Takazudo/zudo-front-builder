...
|---|---|---|
| `init` | `() => Promise<void>` | Eagerly instantiate the wasm module. Optional — every other call function lazily initializes on first use. |
...
Eagerly instantiate the wasm module. Calling it up front only controls **when** the fetch/compile cost happens — every other call function initializes lazily on first use if `init()` was never called.
...
The package is **ESM-only** (`"type": "module"`, no `require` support), requires **Node ≥ 20.0.0**, and is currently at version **`0.1.0-next.95`**.
