...
a single shared islands bundle for the project and injects it only on
pages that contain island markers. A page with one island may download
code for the project's other islands too; pages with no islands skip the
islands script, though other client-script paths can still ship JavaScript.
