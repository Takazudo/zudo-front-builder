...
Workers Static Assets — and it stays a valid Cloudflare Pages advanced-mode
entry too, so the same emitted file deploys unchanged to either target
(dual-compat). Its job is to set up an `AsyncLocalStorage` scope for
