...
3. **Watcher** — `crates/zfb-watcher`. Dev mode only. Observes `pages/`, `content/`, `components/`, `layouts/`, `styles/`, `data/`, and `zfb.config.ts` by default, plus each configured collection's path when it falls outside the default roots. `public/` is deliberately **not** watched — static assets are served live from disk on request, so they need no rebuild and feed neither the watcher nor boot (see [Static Assets](./static-assets.mdx#dev--prod-parity)). Emits a stream of change events.
