...
Framework](/concepts/engine-vs-framework) for the full list. Sitemap and
...
[Frontmatter](/concepts/frontmatter) for the literal-only contract.
...
phase — see [Build Pipeline](/concepts/build-pipeline) — and is
...
- [Frontmatter](/concepts/frontmatter) — the literal-only contract
  for `extension` and `contentType` exports.
- [Routing](/concepts/routing) — the static-route side, including
  the filename convention table.
- [Build Pipeline](/concepts/build-pipeline) — where stale-output
  cleanup runs in the build sequence.
- [Engine vs Framework](/concepts/engine-vs-framework) — why
