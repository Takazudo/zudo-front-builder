...
zfb owns Tailwind end to end: the compiler, the version it runs, and the resolution of every `@import "tailwindcss*"` specifier. The four points below are the whole contract.
...
zfb's CSS import walker recognises the `tailwindcss` prefix and skips resolution entirely (`is_virtual_specifier` in `crates/zfb-css/src/css_imports.rs`); the embedded binary supplies those stylesheets itself. A `node_modules/tailwindcss` directory is therefore never read. Keeping one around is unsupported: it does not change the emitted CSS, does not override the embedded version, and only adds weight to your lockfile.
