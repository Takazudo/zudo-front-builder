...
Once Tailwind is on, utility classes work in `.tsx` files under the scanned content roots (see [Where Tailwind looks](#where-tailwind-looks) below):
...
The aliased target becomes a "mirror root": the CSS pipeline treats a claimed workspace-sibling directory the same way it treats `pages/`, `components/`, and the other scanned roots described in [Where Tailwind looks](#where-tailwind-looks) above.
