...
Utility classes work in `.tsx` files under the scanned content roots (see [Where Tailwind looks](#tailwind-v4-where-tailwind-looks) below):
...
Setting `tailwind: { enabled: false }` switches to an authored-CSS path with no
Tailwind `@import`, utility scan, preflight reset, or subprocess. zfb bundles
resolvable local and package `@import`s with lightningcss, preserves and hoists
external imports, and fails the build when a local or package import is missing.
CSS Modules compilation, class-name hashing, and asset emission all keep working
unchanged; only the Tailwind-specific steps are skipped.
...
removes it once the build finishes. New scaffolds ignore it automatically. If your
project was scaffolded before this glob shipped, add
...
- Plain `.css` imports from JavaScript/TypeScript modules are not emitted. zfb warns when esbuild resolves one; `strictPlainCssImports: true` (or `zfb build --strict-plain-css-imports`) turns the warning into a build error. Put global CSS in `styles/global.css` (and use `@import` there), or use a default-imported `.module.css` file for scoped component CSS.
