...
watched — nothing under `node_modules` is, by design. Replacing one while the
dev server is running produces no watcher event, so CSS is never recompiled and
the served content-hashed companion keeps handing out the old bytes until
something else triggers a CSS rebuild — restart `zfb dev`, or touch the
stylesheet that `@import`s the package, to pick the new file up (see
...
watched set is a deliberate scope decision — a third dynamic watch-registration
channel carries real watcher-narrowing risk.
