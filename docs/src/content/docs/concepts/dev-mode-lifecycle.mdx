...
watched. Replacing one inside `node_modules` while the dev server is running
...
`@import`s the package, to pick the new file up (see
[Static Assets — CSS imported from a package](./static-assets.mdx#css-imported-from-a-package-relative-asset-references)).
This rarely bites in practice, since package assets change through
`pnpm install`, which churns paths the watcher already watches, and keeping
arbitrary `node_modules` asset files out of the watched set is a deliberate
scope decision — a third dynamic watch-registration channel carries real
watcher-narrowing risk.
