...
using `[param]` or `[...param]` brackets in `.md` or `.html` file names
are **not supported** — the scanner skips them with a warning and
produces no output pages. There is no `paths()` story for these
extensions in v1 (`.tsx` cannot be `export`-ed from a plain Markdown or
HTML file). If you need dynamic routes, author them as `.tsx` pages
instead. Static (non-parameterised) `.md` and `.html` page paths work
correctly.
