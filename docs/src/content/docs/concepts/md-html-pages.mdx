...
zfb accepts seven page source extensions in total — `.tsx`, `.ts`, `.jsx`,
`.js` (script pages, all bundled the same way) plus `.mdx`, `.md`, and
`.html` (covered on this page). All seven follow the same file-system
routing conventions under `pages/` — `pages/about.md` and `pages/about.tsx`
both produce the route `/about`. What differs is the render path each
extension takes. `.mdx` shares the MDX pipeline with `.md` and accepts the
same frontmatter keys (see the `.md` pages section below). See
[Routing](./routing.mdx) for the full accepted-extensions list.
...
`.md`, `.mdx`, and `.html` pages are build-time (SSG) page sources. The
dynamic-route skip gate only applies to `.md` and `.html`: bracketed
`.md` / `.html` filenames are skipped with a warning because they cannot
export `paths()`. A bracketed `.mdx` page is accepted by the scanner and
can use the MDX module export surface for `paths()`. Static
...
- [Routing](./routing.mdx) — the full file-to-route mapping table.
- [Frontmatter](./frontmatter.mdx) — the unified frontmatter
  contract for `.tsx` pages (includes TSX literal-only rules and
  output-extension precedence).
- [Non-HTML Pages](./non-html-pages.mdx) — emit XML, JSON, or
  plain text from `.tsx` pages.
- [Content Collections](./content-collections.mdx) — the
