...
admonition-style directives (see the [Admonitions recipe](../recipes/admonitions.mdx)).

If you only need to register directives from config without writing Rust, use
the opt-in [`directives`](../markdown-features/directives.mdx) feature in
...
- **Provide any default directive set.** No built-in `:::callout`, `:::card`,
  ...
  the [`directives`](../markdown-features/directives.mdx) config feature or the Rust
  API.
- **Auto-import the JSX components.** You're responsible for making
  ...
  [MDX Components](./mdx-components.mdx) for the
  `defaultComponents` recipe.

## See also

- [`directives` feature](../markdown-features/directives.mdx) — register directive names from `zfb.config.ts` without writing Rust.
- [Recipe: Admonitions](../recipes/admonitions.mdx) — a worked example using the `directives` feature to register `:::note`, `:::tip`, etc.
- [MDX Components](./mdx-components.mdx) — how compiled MDX
  modules look up element-level overrides.
- [Engine vs Framework](./engine-vs-framework.mdx) — why the
