...
This build-time/request-time symmetry, and a third run shape in which another
program spawns `zfb dev` as a local content server, are described in
[Three ways to run zfb](./three-ways-to-run-zfb.mdx).
