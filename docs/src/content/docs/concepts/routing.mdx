...
- Accepted page extensions are `.tsx`, `.ts`, `.jsx`, `.js`, `.mdx`, `.md`, and `.html`. `.tsx` is not the only script-page shape — a page with no JSX markup can be written as plain `.ts` (or `.jsx`/`.js`) using `h()` calls directly, and it routes exactly like `.tsx`. Files with any other extension in `pages/` are skipped (a warning is logged), so README files and ad-hoc notes are safe to drop there.
