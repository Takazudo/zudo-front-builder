...
`paths()` is evaluated by the embedded V8 host during the build. `zfb build` discovers static, dynamic, and catchall routes and evaluates `paths()` for each dynamic and catchall route to enumerate the concrete URLs to render. Static routes require no `paths()` export. `paths()` is evaluated once per route per build — per-entry work inside it is safe and will not be repeated for each output URL.
