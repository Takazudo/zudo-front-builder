...
```tsx
import { navigate } from "@takazudo/zfb-runtime/client-router/router";
...
- **`history: "push"`** (or `"auto"`, the default) — creates a new browser history entry.
...
event.newDocument     // Document  — populated after fetch (writable)
...
} from "@takazudo/zfb-runtime/client-router/router";
