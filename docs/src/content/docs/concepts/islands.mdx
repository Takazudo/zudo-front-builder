...
During `zfb dev`, raw targets are watched independently of their importers.
Editing, deleting, or recreating the text file invalidates the browser bundle;
the same transform and dependency tracking also apply to client scripts and
the page/SSR pass.
...
Tailor the directives to what your worker actually accesses. If `base` is set,
include that prefix in the `_headers` path. Workers owned by
