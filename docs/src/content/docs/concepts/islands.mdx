...
| A module transitively imported from a `"use client"` island | Yes, since #1404, same restrictions as above |
...
Any of the following — found anywhere in the module graph reachable from a `"use client"` island — fails `zfb build` with a message naming the offending file. During `zfb dev` the same message is logged as a warning and that rebundle tick is skipped, so the dev server stays up while you fix the file and save again:
