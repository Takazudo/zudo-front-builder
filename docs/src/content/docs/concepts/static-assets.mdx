...
zfb resolves that reference against the stylesheet's own location, emits the referenced file as a content-hashed asset beside the compiled CSS — `{stem}-{hash8}.{ext}` in `dist/assets/` — and rewrites the `url()` to a relative reference pointing at it: `./{stem}-{hash8}.{ext}`. Importing the same file twice emits it once. A `?query` or `#fragment` suffix on the original reference is preserved on the rewritten one. This works for any file type a package stylesheet references — fonts, images, anything — not fonts only.

**Your own authored CSS is unaffected.** The global entry, and any project-local `@import`ed stylesheet, keep the [absolute-URL contract](#referencing-assets) above unchanged — a relative `url()` written in your own CSS is passed through untouched, exactly as before. `public/` copying, and `data:` / absolute / full URLs anywhere, are never touched by this rewrite either. The distinction is by **where the stylesheet comes from**, not by anything you opt into: your CSS keeps working the same way it always has, and a package's CSS gets asset emission because a package has no other way to ship files alongside itself.
