...
**`node_modules` importers.** A `to` that resolves to a single existing file is also applied to importers inside `node_modules`, via esbuild's `--alias` flag. A directory-shaped or missing `to` is applied through tsconfig `paths` only, which esbuild does not honor for `node_modules` importers — so a package that needs the alias to work from inside `node_modules` should register one alias per file, or use `addVirtualModule` instead. A `from` that is a slash-prefix of another registered alias, of a virtual module specifier, or of a reserved name such as `zfb` gets no `--alias` flag.
...
A pattern whose first segment is `__paths__` and which has at least one further segment is reserved for zfb's internal `paths()` endpoint. The route scan rejects it with `RouterError::ReservedRoutePrefix`, and the error identifies the plugin that registered the pattern. Exactly `/__paths__` and patterns with `__paths__` in a later segment remain valid.
