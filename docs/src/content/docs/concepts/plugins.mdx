...
**v1 caveat: exact-match-only.** Registering `@/foo` does **not** match imports of `@/foo/bar`. There is no prefix matching in v1 — you can't register `@/` once and have it route everything under it. Each aliased file gets its own `addAlias` call. Prefix-matching is deferred to a future revision.
