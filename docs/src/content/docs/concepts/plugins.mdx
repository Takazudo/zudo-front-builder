---
title: Plugins
description: Author and consume zfb plugins — the four lifecycle hooks, virtual modules, import aliases, injected routes, and client entries.
sidebar_position: 12
category: Concepts
---
...
For responses that are not pages, use the separate `devMiddleware` hook: a JSON API endpoint, a hot-reload bridge, an upload handler. No JSX pipeline, no page renderer — just a function returning `{ status, headers, body }`.
