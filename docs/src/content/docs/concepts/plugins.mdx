...
- **A failed re-invoke keeps the last good source.** If the re-invoked loader throws — a half-written file, JSON caught mid-save — zfb retains the previously memoised source, logs the error, and keeps watching. The next change that loads cleanly replaces the memo, so deleting and recreating a file recovers on its own.
