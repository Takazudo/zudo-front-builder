...
Registers a TSX/TS file as a synthetic page route the dev server evaluates through the **page rendering pipeline**, the same way it evaluates `pages/<...>.tsx`. The pattern follows the `pages/` filename grammar (`/blog/[slug]`, `/api/dev/x`, `/docs/[...rest]`); `entrypoint` is resolved relative to the project root.
