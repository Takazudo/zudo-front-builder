...
A bundle failure surfaces as `` plugin bundling: esbuild failed for plugin `{name}` … `` followed by esbuild's own diagnostics (including file:line:col). A plugin whose code throws at load or during its `setup`/lifecycle hooks still surfaces as the familiar plugin `init`/hook error — bundling failures and runtime failures are reported distinctly.
...
- **A failed re-invoke keeps the last good source.** If a re-invoked loader throws — a half-written file, JSON caught mid-save — zfb retains the previously memoised source, logs the error, and keeps watching. The refresh is all-or-nothing: when one change re-invokes several loaders and any of them fails, none of the fresh sources are published, so a run never serves a half-updated mix. The next change that loads cleanly replaces the memo, so deleting and recreating a file recovers on its own.
