...
How to replace the removed `imageEnlarge` built-in feature by wiring a userland `EnlargeableImg` component through the [MDX override API](../concepts/mdx-components.mdx). Both a fully server-side variant (no JavaScript shipped per page) and an islands variant (client-side interactivity via `<Island>`) are shown.
...
- [MDX Components](../concepts/mdx-components.mdx) — the `components` prop, `defaultComponents`, and the `mdx-components.tsx` global convention.
- [Islands](../concepts/islands.mdx) — the `<Island>` wrapper, hydration strategies, and the `"use client"` directive.
