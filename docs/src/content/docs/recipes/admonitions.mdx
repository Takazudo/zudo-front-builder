...
How to register an admonition vocabulary (`note`, `tip`, `info`, `warning`, `danger`, `caution`) and a collapsible `details` directive using the generic [`directives`](../markdown-features/directives.mdx) opt-in feature. Includes minimal component stubs and CSS patterns. [zudo-doc](https://github.com/zudolab/zudo-doc) is the fully-worked reference implementation.

</Info>

## Background

zfb is the **engine** — it provides the [Directives registry](../markdown-features/directives-registry.mdx) as a Core primitive and the [`directives`](../markdown-features/directives.mdx) opt-in feature to populate it from config. The engine ships **zero** named directives by default. The admonition vocabulary (`:::note`, `:::tip`, etc.) is a userland policy decision — this recipe shows one way to implement it.
...
The pipeline emits `<Note title="Optional title">`, `<Tip>`, `<Details title="Click to expand">`, etc. You are responsible for making these identifiers available in scope — see [MDX Components](../concepts/mdx-components.mdx).
...
Add the components to your [MDX components map](../concepts/mdx-components.mdx) so they are in scope at every page render.
...
- [`directives` feature reference](../markdown-features/directives.mdx) — the Opt-in feature this recipe uses.
- [Directives registry](../markdown-features/directives-registry.mdx) — the Core primitive underneath.
- [MDX Components](../concepts/mdx-components.mdx) — how to put the components in scope globally.
- [Custom Directives](../concepts/custom-directives.mdx) — register directives via the Rust API for framework authors.
