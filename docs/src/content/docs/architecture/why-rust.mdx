...
The first `cargo build` of zfb's workspace is slow. The V8 source bundle (pulled in by `zfb-render` via `deno_core`) compiles for 15 to 30 minutes on a typical contributor machine. Incremental builds are fast; the first one hurts. See [JS runtime](/architecture/js-runtime) for the runtime details and [ADR-007](https://github.com/Takazudo/zudo-front-builder/blob/main/docs/architecture/adr-007-embedded-v8.md) for why this cost is accepted.
