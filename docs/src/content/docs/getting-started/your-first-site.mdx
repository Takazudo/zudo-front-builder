...
`--template basic-blog` is the default, so you can omit it. A `node-free` template is also available for projects that need no Node.js dependency — see [Install without Node](/install/node-free) for details.
