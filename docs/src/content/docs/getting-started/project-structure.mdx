...
The default template ships an index page and three dynamic routes: `blog/[slug].tsx` (one HTML file per post), `blog/page/[page].tsx` (paginated listing), and `tags/[tag].tsx` (per-tag archive). A dynamic route file exports a `paths()` function; zfb calls it at build time to expand the parameter into one HTML file per resolved value. See [Dynamic routes](/concepts/dynamic-routes) for the full API.
