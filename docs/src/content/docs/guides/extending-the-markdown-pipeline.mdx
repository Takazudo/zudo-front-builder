...
Examples: all 14 features in `zfb-md-extras`.
...
flags are set. That is how all 14 features in `zfb-md-extras` are wired.
...
- **`ImageEnlargePlugin` and `ImageDimensionsPlugin` are
  order-independent** relative to the code pipeline — they only
  touch `<img>` elements.
