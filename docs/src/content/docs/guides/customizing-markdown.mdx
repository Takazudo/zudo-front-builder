...
[Markdown Features](../markdown-features/index.mdx) category.
...
system. Each registered plugin exposes five optional hooks — `setup`,
`preBuild`, `postBuild`, `devMiddleware`, and `previewMiddleware` — for tasks like registering
...
(`postBuild`), or adding routes to the dev server or static preview server
(`devMiddleware` / `previewMiddleware`). Plugins
...
- [Markdown Features](../markdown-features/index.mdx) — full catalog of Core and Opt-in
  features.
