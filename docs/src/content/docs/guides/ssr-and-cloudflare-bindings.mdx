...
The SDK declares the default import as `WebAssembly.Module`; it is not an
instantiated module or an untyped URL string. The starter's page files already
import `@takazudo/zfb`, which makes that ambient declaration part of the
TypeScript program. If a project keeps Wasm-only helpers outside that import
graph, add an included declaration-only bridge instead of duplicating the
module declaration:
...
For example, make sure the file is covered by your `tsconfig.json` `include`
list when it does not already include all of `src/`.
