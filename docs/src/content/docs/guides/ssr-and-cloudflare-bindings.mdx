...
- `_worker.js` + `_zfb_inner.mjs` — the Worker entry (a thin wrapper
  ...
  routes, and
- `.assetsignore` — lists `_worker.js` and `_zfb_inner.mjs` so the
  asset server never serves your server code as a public file (a request
...
<Note title="Still deployable to Cloudflare Pages">

The same `dist/` output remains deployable to **Cloudflare Pages
advanced mode** — a root-level `_worker.js` is Pages' equivalent
convention, and the wrapper dispatches identically on both platforms.
Workers Static Assets is the primary, recommended target; Pages advanced
mode stays supported for existing Pages projects. (The only
platform-visible difference is a trailing-slash asset redirect: `307` on
Workers Static Assets versus `308` on Pages.)
