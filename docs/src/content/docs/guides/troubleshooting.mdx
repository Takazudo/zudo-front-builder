...
**Symptom:** the [curl installer](../install/node-free.mdx#linux-macos-—-curl) exits with one of:
...
- With `ZFB_VERSION` unset, it asks for the latest **non-prerelease** release. Pre-1.0, zfb has shipped only prereleases (`v0.1.0-next.N`) — there is no stable release to resolve yet, so this fails by design. See the note in [Install without Node](../install/node-free.mdx#linux-macos-—-curl).
...
- Pre-1.0 (no stable release exists yet): explicitly opt into prereleases —

  ```sh
  curl -fsSL https://raw.githubusercontent.com/Takazudo/zudo-front-builder/main/install.sh | ZFB_VERSION=latest-prerelease sh
  ```

- If `latest-prerelease` itself fails to resolve: pin an exact tag from the [releases page](https://github.com/Takazudo/zudo-front-builder/releases) instead, e.g. `ZFB_VERSION=v0.1.0-next.76`.
