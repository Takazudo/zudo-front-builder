...
Five of the six packages a default install pulls are affected. `@takazudo/zfb-darwin-x64` is not flagged only because it has had no attestation since well before `2.12.0`, so there is nothing for pnpm to compare against.
