...
- With `ZFB_VERSION` unset, it asks for the latest **non-prerelease** release. This normally resolves; it fails only if the API is unreachable, rate-limited, or the request is otherwise blocked. See [Install without Node](../install/node-free.mdx#linux-macos-—-curl).
