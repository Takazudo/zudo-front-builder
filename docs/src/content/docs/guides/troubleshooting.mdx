...
**Symptom:** installing or upgrading any `@takazudo/zfb*` package at `2.12.0` fails under pnpm's opt-in [`trustPolicy`](https://pnpm.io/settings/dependency-resolution#trustpolicy) set to `no-downgrade`:
...
| `pnpm install --frozen-lockfile` | succeeds | fails |
...
3. **Exempt just these packages**, if you need `2.12.0` now, with [`trustPolicyExclude`](https://pnpm.io/settings/dependency-resolution#trustpolicyexclude) in `pnpm-workspace.yaml` — it keeps the policy active for every other dependency:
...
## `ERR_PNPM_TRUST_DOWNGRADE` installing `@takazudo/zfb-darwin-x64` 2.20.3

**Symptom:** installing or upgrading `@takazudo/zfb-darwin-x64` at `2.20.3` fails under pnpm's opt-in [`trustPolicy`](https://pnpm.io/settings/dependency-resolution#trustpolicy) set to `no-downgrade`:

```text
ERR_PNPM_TRUST_DOWNGRADE High-risk trust downgrade for "@takazudo/zfb-darwin-x64@2.20.3" (possible package takeover)
```

Because pnpm resolves all `optionalDependencies` regardless of the host platform, this affects consumers on any OS, not just macOS x64 hosts.

**Cause:** `2.20.3` was released using the `--fast-mac` escape hatch, which publishes `zfb-darwin-x64` without `--provenance`. Every version from `2.13.0` onward (when this package's attestation gap was first closed — see the `2.12.0` entry above) through `2.20.2` carried provenance, so `2.20.3` is a genuine trust downgrade for pnpm's date-based comparison, not a standing gap. This was not caught before publishing; the preflight check and release-skill gating that now block this case did not exist yet.

**Fix:** npm versions are immutable, so `2.20.3` itself can never gain an attestation. Prefer, in order:

1. **Upgrade to `2.20.4` or later.** Those releases go out through the normal provenance-first CI path with attestation restored, which clears the downgrade. This is the real fix.
2. **Pin `2.20.2`** if upgrading is not yet possible — it remains attested and avoids the affected `2.20.3` release.
3. **Exempt just this package**, if you need `2.20.3` now, with [`trustPolicyExclude`](https://pnpm.io/settings/dependency-resolution#trustpolicyexclude) in `pnpm-workspace.yaml`:
...
Disabling `trustPolicy` outright also works, but drops the guard for **every** dependency rather than this one — prefer the scoped exemption.
...
| Variable | Scope | What it keeps |
|---|---|---|
| `ZFB_KEEP_BUILD_SHADOW=1` | `zfb build` only | The bundler's shadow root, its exact-`node_modules` isolation root, and the package-routes overlay (when the project registers package routes). Survives a failing build too. Each kept path is printed once to stderr. See [`zfb build`: Environment variables](../api/cli.mdx#zfb-build). |
| `ZFB_KEEP_SHADOW_SESSION=1` | `zfb dev` only | The dev server's one persistent shadow session tempdir for the whole process. A later `zfb dev` boot without the flag reaps a previously-kept dir once it passes an internal age floor — keep the flag exported for the whole debugging session, not just the boot that created the dir. See [`zfb dev`: Environment variables](../api/cli.mdx#zfb-dev). |
