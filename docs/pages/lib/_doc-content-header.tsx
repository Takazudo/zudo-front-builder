...
export function DocContentHeader({
  ...
  return (
    <>
      ...
      {/* Feature tier badge — "Core" / "Opt-in" — shown under the h1 on the
          markdown-features pages that set the `tier` frontmatter key. Ported
          from the pre-zfb [...slug].astro route (issue #877 step 6). Placed in
          this shared header so it serves both the default and [locale] slug
          routes from a single source. */}
      {entry.data.tier && (
        <span
