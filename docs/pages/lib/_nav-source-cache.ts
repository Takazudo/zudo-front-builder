...
// installs once at worker boot. We memoize the bridged + draft-filtered arrays
// on that anchor (WeakMap) so every repeat caller receives the SAME array
// instance. `buildNavTree`'s WeakMap fast-path (`src/utils/docs.ts`) then
// short-circuits the key computation on identity.
//
...
// to key on, so we DELIBERATELY do not memoize the no-snapshot path.
//
// PACKAGE SPLIT (docs-1.0-migration, S3):
