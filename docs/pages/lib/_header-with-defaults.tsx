 ...
 */
function remapVersionedHrefs(nodes: NavNode[], version: string, nodeLang: Locale): NavNode[] {
...
export function HeaderWithDefaults(props: HeaderWithDefaultsProps): JSX.Element {
  const { lang = defaultLocale, currentPath = "", currentVersion, currentSlug, navSection } = props;
  ...
  const localeLinks = locales.length > 1 ? buildLocaleLinks(currentPath, lang) : undefined;

  const themeDefaultMode = settings.colorMode ? settings.colorMode.defaultMode : undefined;
  ...
  if (navSection !== undefined) {
    ...
    sidebarNodes = currentVersion ? remapVersionedHrefs(rawNodes, currentVersion, lang) : rawNodes;
  }
