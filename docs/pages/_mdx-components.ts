...
// now ships from `@takazudo/zudo-doc/mdx-components` (docs-1.0 migration,
// S5 #1249). This file keeps ONLY what is genuinely project-bound:
//
//   - the locale-bound nav wrappers (CategoryNav / CategoryTreeNav /
//     SiteTreeNav) — they depend on the project's content collections + nav-tree
//     utilities, so they cannot live in the package. They are passed into the
//     factory as `navData`; the factory injects the active `locale` so `/ja`
//     resolves the JA collection (the load-bearing locale thread — a static
//     global mdx-components slot would render the EN tree on every `/ja` page,
//     which is why this stays a per-call factory). The factory reuses
//     `SiteTreeNav` for the `<SiteTreeNavDemo>` corpus tag.
 ...
 *   - `navData`: the locale-aware nav wrappers (the factory injects `lang`).
