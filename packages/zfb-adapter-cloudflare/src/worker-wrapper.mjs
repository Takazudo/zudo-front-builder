...
//     inner ALSO 404s with only a generic body (Hono's default
//     \`text/plain\` "404 Not Found", or \`text/html\`), the earlier styled
//     asset 404 is preferred over the inner's plain 404 — otherwise the
//     styled \`404.html\` would be discarded and users would see the inner
//     plain-text 404 (issue #1322). An inner 404 that declares a
//     structured payload (\`application/json\`, etc.) is treated as an
//     intentional API 404 and returned unchanged. Under
//     \`not_found_handling = "none"\` the asset 404 body is empty/non-HTML,
//     so the inner always wins — the historical behavior is preserved.
...
function innerHasIntentional404Payload(response) {
  // Distinguishes an *intentional* inner API 404 (which must survive) from
  // the framework's generic not-found (which yields to the styled asset
  // page). The inner zfb router never overrides Hono's default handler, so
  // a route miss is always \`text/plain\` "404 Not Found"; a custom
  // \`pages/api/*.tsx\` 404 returns its own Response with a declared
  // content-type (idiomatically \`application/json\`). Treat any 404 whose
  // content-type is neither the \`text/plain\` default nor \`text/html\` as an
  // intentional machine-readable payload. Trade-off: a bare \`text/plain\`
  // API 404 is indistinguishable from the default and will yield to the
  // styled page — API errors should use \`application/json\` to be preserved.
  const contentType = (response.headers.get("content-type") || "").toLowerCase();
...
export default {
  async fetch(request, env, ctx) {
    if (isAssetProbeMethod(request.method) && canDelegateToAssets(env)) {
      ...
      // styled 404 page: if the inner also 404s with only a generic body,
      // we return this styled page instead of the inner's plain 404
      // (issue #1322). Returned verbatim — the one-shot body is untouched.
      ...
      if (
        ...
        !innerHasIntentional404Payload(innerResponse)
      ) {
