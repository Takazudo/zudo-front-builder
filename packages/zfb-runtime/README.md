...
> **Status: 0.1.0-migration.0.** Wave-1 deliverable for the
> [SSG-render epic][epic-52]. The package is published source-first
> (workspace-internal); the eventual public-registry publish will add a
> `dist/` build step. Until then `exports` points at `./src/*.ts`,
> matching the `zfb` package convention.

[epic-52]: https://github.com/Takazudo/zudo-front-builder/issues/52
...
interface PageModule {
  ...
  readonly prerender?: boolean;          // T5: literal `false` excludes from SSG
  readonly content_type?: string;        // overrides Content-Type (e.g. "application/xml")
  readonly headings?: readonly PageHeading[]; // T4: MDX-emitted TOC data
}
...
```ts
// dist/worker.mjs (shape — generated by T3, not committed)
import { createPageRouter } from "@takazudo/zfb-runtime";
...
T6 then drives the Worker by sending `GET` requests for each enumerated
route and writing the response body to `dist/{route}/index.html`.

### What stays stable for T6

The contract this package commits to:

- `createPageRouter` is the single export T6 wires to.
...
- `Content-Type` defaults to `text/html; charset=utf-8`. Page modules
  with a `content_type` field override it (per shipped #49).
- Errors in page evaluation surface as 500 responses with a diagnostic
  text body. T6's source-map plumbing projects those back to the user's
  TSX line.
- The `ContentSnapshot` registration is idempotent and observable via
  `getContentSnapshot()` re-exported from `zfb/content`. T6 dev mode
