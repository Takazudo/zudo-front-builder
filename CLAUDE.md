...
| **T1 — PR gate** | `.github/workflows/health.yml` — fmt, `clippy -D warnings`, build, `cargo nextest run --workspace --profile ci` + a separate `cargo test --workspace --doc`, `pnpm test:workspace`, `format:check`, actionlint, build-no-v8. | **the authoritative gate.** A PR is mergeable when its required checks are green |
