...
**The root corpus is split into two vitest projects** by `vitest.workspace.mjs` (#3058/#3061): `scripts` keeps vitest's 5000 ms `testTimeout` as a hang guardrail, and `scripts-subprocess` raises it to 60 s for the four suites that `await` real child processes — measured, not guessed. A suite belongs there only if it *awaits* a subprocess: vitest's `testTimeout` is a timer, so an `execFileSync`-blocking test cannot be interrupted by it at any host load. Add such a suite to the single `SUBPROCESS_SUITES` list in that file and nowhere else — both projects are carved out of it, because `extends` merges arrays by concatenation and a project-level `include` would be *appended* to the root pattern rather than replace it, running every other suite twice.
...
- **`cargo-nextest` (ADOPTED — #1340):** CI runs the Rust suite under `cargo nextest run --workspace --profile ci` (health.yml), configured in `.config/nextest.toml`. The `ci` profile RECORDS retries (`retries = 1`, pass-on-retry reported as `FLAKY` + the `nextest-junit-ci` artifact; #1341 consumes that telemetry) — do not raise `retries` to paper over a flake; local runs use the default profile (`retries = 0`) so flakes fail loudly. **Doctests are NOT run by nextest** — health.yml keeps a separate `cargo test --workspace --doc` step. The doctest baseline, the two `e2e-heavy-*` groups (25 flock-adopting binaries in `e2e-heavy-locked` and 18 build-only binaries in `e2e-heavy-unlocked`, each `max-threads = 1`), the required-features lane, and the inventory-parity reconciliation rule all live in `crates/CLAUDE.md`.
- **`scripts/__tests__/docs-dev-supervisor.test.mjs` phase telemetry (#2889/#2904):** opt-in `[supervisor-timeline]` lines behind `ZFB_SUPERVISOR_TIMELINE=1`, off by default (b4push does not set it — extra stderr noise the local loop didn't ask for). `health.yml` now sets it on the `pnpm test:workspace` step so every ubuntu CI run contributes a sample. This buys observability, not storage — lines land scattered across workflow logs under GitHub's retention. `scripts/supervisor-timeline-summary.mjs` reads them and applies #2887's R-A/R-B/R-C rules; its doc comment is the protocol's source of truth. `scripts/harvest-supervisor-timelines.mjs` (#2908) is the aggregation step: it re-fetches each run's `health` job log via `gh` and re-parses it with the summarizer's own `parseTimelines` on demand, so nothing is persisted between runs; its `--save-dir` keeps the raw job logs, which the weekly watch folds into a single `job-logs.tar.gz` (both passes are done reading the plain directory by then, and the source logs are only deleted once the archive is verified — a broken archive aborts the script instead of shipping an artifact that has grown, #2934) and uploads as the artifact an R-A triage reads and reuses for its `main`-only pass. Since #2931 both per-run fetches are direct REST calls — `gh api repos/{owner}/{repo}/actions/runs/<id>/jobs?per_page=100` and `gh api repos/{owner}/{repo}/actions/jobs/<id>/logs`, one metered GET each (the log endpoint's 302 to a plaintext blob is a separate, unmetered host, and gh expands `{owner}`/`{repo}` locally). The `gh run view` porcelain it replaced cost ~7 GETs per run — measured with `GH_DEBUG=api` — which put a capped 200-run harvest near `GITHUB_TOKEN`'s 1,000-per-hour primary limit, where it would 403 mid-harvest and file a tracking issue about its own rate limit; the same harvest is now ~400. The REST body is not byte-identical to the porcelain's: it has no `<job>\t<step>\t` prefix and opens with a UTF-8 BOM, so `record.raw` and the `--save-dir` bytes changed while the parsed records did not. `scripts/__tests__/fixtures/rest-job-log-capture.log` is a trimmed real capture of that body (GitHub owns the envelope, so it is captured, not hand-authored — unlike `supervisor-timeline-samples.txt`, which is zfb's own emitter contract), and both suites build their synthetic logs from its prefix. The `env=` token is a steering-only digest over `STEERING_ENV_KEYS` (`scripts/supervisor-env-identity.mjs`, #2913), carried on the wire as `env=v<N>:sha256:<16 hex>` where `<N>` is `ENV_IDENTITY_VERSION` — the version of the *contract* that produced the digest (#2933). Two digests are comparable only under the same version, so changing `STEERING_ENV_KEYS`, its order, or the serialization is a version bump rather than a new date epoch; an unversioned `env=sha256:…` record predates the prefix and reads as **v1**, so a population straddling #2933 shows no drift. The summarizer groups each case's records into one cohort per version: **R-A (failed-record detection) stays global** across versions, while identity drift, R-B, and the four timing distributions are evaluated **per cohort and never pooled**. `--strict` fails when any cohort trips R-B or carries disallowed within-version drift; a version split is reported prominently but is **not** itself a strict failure, because it heals as the old cohort rolls out of the harvest window. `--allow-drift env` therefore means "tolerate two different steering digests under the *same* contract version" and says nothing across cohorts; it is not the remedy for a contract bump, and the retired `IDENTITY_CONTRACT_EPOCH` date floor no longer exists. The harvester defaults to a rolling 8-day window (one weekly cadence plus a day of overlap, so a run in progress at one harvest is still enumerated by the next). The repository's 7-day Actions retention expires job logs before that window closes, so the overlap day's runs are enumerated but skipped as `log-expired` (HTTP 410), never a partial harvest; raising the retention to >= 9 days in the repo settings (UI-only; there is no REST endpoint) would restore the overlap's coverage. It prints the effective window and warns when `--since` lies in the future, keeps "nothing harvestable" (exit 4) apart from "green `health` jobs with zero records" (exit 1), and its per-run manifest lines name runs carrying `failedRecords=` records — a distinct counter from the final summary line's `failed=` (runs the harvester itself could not fetch/parse), renamed apart in #2932 so no `^run=` anchor is needed to tell them apart. Its gh-retry pause (2000 ms default) is overridable via the `HARVEST_RETRY_DELAY_MS` env var (test ergonomics only; `scripts/supervisor-watch.mjs` forwards its `WATCH_RETRY_DELAY_MS` setting, #2932); `supervisor-watch.yml` automates the R-A/R-B watch weekly. The job-log fetch adds `--allow-escape-sequences` when a one-time `gh api --help` capability probe (once per `runCli` invocation) says the installed gh (>= 2.97) advertises it, since a probe failure or an older gh falls back to the pre-2.97 behavior without it (#2986).
