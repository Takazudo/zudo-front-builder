...
- **`cargo-nextest` (ADOPTED — issue #1340):** CI runs the Rust test suite under `cargo nextest run --workspace --profile ci` (health.yml), configured in `.config/nextest.toml`. This gives the CI retry budget, per-test timeouts, and JUnit output — the mechanisms below are now real, not aspirational.
  ...
  - **Heavy e2e binaries are serialized by a nextest `test-group`.** nextest schedules test *binaries* concurrently by default, which would boot several real `zfb dev`/`zfb build`/`zfb preview` V8+esbuild processes at once. `.config/nextest.toml` defines a `[test-groups.e2e-heavy]` with `max-threads = 1` and an override assigning **18** heavy binaries to it: **10 flock-adopting** binaries (`build_terminates`, `client_bundling_cross_pipeline`, `dev_bind_before_walk_e2e`, `dev_build_static_parity`, `dev_dep_invalidation_1284_e2e`, `dev_out_of_root_collection_e2e`, `dev_public_large_tree_smoke_e2e`, `dev_serve_e2e`, `dev_serve_injected_routes_e2e`, `preview_cross_mode_e2e`) that spawn `zfb dev` and/or `zfb build` and/or `zfb preview`, plus **8 build-only** binaries (`build_cleans_outdir`, `build_package_routes`, `build_package_routes_consumer`, `client_router_autoinclude_build`, `content_snapshot_no_deferred`, `css_modules_components_build`, `end_to_end_basic_blog_build`, `html_minify_build`) that do not adopt the flock. For the first 10, the flock and test-group are **defense in depth** (flock = process-level guarantee; test-group = runner-level guarantee); for the build-only 8, the test-group is the **sole** serialization guard. `preview_cross_mode_e2e` (issue #1547) runs a real `zfb build` and then spawns both a real `zfb dev` and a real `zfb preview` (static) against that build output, proving the `_redirects`/plugin-middleware behavior landed by epic #1541 is identical across both. `dev_out_of_root_collection_e2e` (issue #1552, epic #1548) is the newest flock-adopting member: it spawns a real `zfb dev` over a collection whose configured root lives outside the project directory (`allowOutsideRoot: true`), proving the wave-2 canonical-path dev-server fix (#1550) narrows re-render to the edited entry's own route (not a full fallback rebuild) and discovers a newly created out-of-root entry without a restart. (`check_command` and `framework_packages_no_pnpm` are deliberately excluded — the former runs only `zfb check`, the latter calls `zfb_build::bundle` in-process without spawning `zfb` or booting V8.) The group also carries a long per-test `slow-timeout` (`terminate-after` = 600s) so legitimate dev-server watchdogs are never killed as "hung". The same override also joins `render_pipeline::tests::eval_deferred_paths_via_worker_embedded_v8_non_literal_paths` and `version_stamp_from_env` by exact test name; this keeps those individual heavy tests serialized against the other 18 binaries when exam.yml runs the ignored manifest.
...
Audited and retagged in full during issue #1337 (2026-07); reconciled again during issue #1504 (2026-07). The table below lists all **27** ignored Rust tests (**22** `env-gate`, **5** `heavy`) and their scheduled homes. Update this table whenever a `#[ignore]` is added, removed, or reclassified.
...
| `crates/zfb-islands/tests/integration.rs:830` `client_script_raw_import_bundles_text` | `env-gate` (esbuild) | Same as above. |
| `crates/zfb-islands/tests/integration.rs:895` `island_module_worker_emits_contract_companion_and_dev_layout` | `env-gate` (esbuild) | Same as above. |
| `crates/zfb-islands/tests/integration.rs:1051` `module_worker_define_only_change_updates_query_and_emitted_bytes` | `env-gate` (esbuild) | Same as above. |
| `crates/zfb-islands/tests/integration.rs:1111` `module_worker_package_config_switch_updates_query_and_emitted_bytes` | `env-gate` (esbuild) | Same as above. |
| `crates/zfb-islands/tests/integration.rs:1203` `module_worker_plugin_inputs_update_query_closure_and_emitted_bytes` | `env-gate` (esbuild) | Same as above. |
| `crates/zfb-islands/tests/integration.rs:1307` `island_css_import_bundles_without_error` | `env-gate` (esbuild) | Same as above. |
| `crates/zfb-islands/tests/integration.rs:1369` `island_module_css_import_bundles_without_error` | `env-gate` (esbuild) | Same as above. |
| `crates/zfb-islands/tests/integration.rs:1641` `islands_shadow_expands_glob_and_executes` | `env-gate` (esbuild) | Same as above. |
| `crates/zfb-islands/tests/integration.rs:1712` `islands_shadow_preserve_symlinks_is_load_bearing` | `env-gate` (esbuild) | Same as above. |
| `crates/zfb/src/commands/build.rs:8386` `preprocessing_shadows_bundle_nested_alias_raw_and_workers_with_real_esbuild` | `env-gate` (esbuild) | **CI**: `health.yml` → "Run zfb command-layer env-gate unit tests (commands::build)". Local: `ZFB_ESBUILD_BIN=<abs path> cargo test -p zfb --lib commands::build::tests::preprocessing_shadows_bundle_nested_alias_raw_and_workers_with_real_esbuild -- --ignored`. |
| `crates/zfb/src/commands/build.rs:8770` `plugin_alias_only_client_preprocessing_triggers_shadow_with_real_esbuild` | `env-gate` (esbuild) | Same CI step as above. Local: `ZFB_ESBUILD_BIN=<abs path> cargo test -p zfb --lib commands::build::tests::plugin_alias_only_client_preprocessing_triggers_shadow_with_real_esbuild -- --ignored`. |
| `crates/zfb/tests/client_bundling_cross_pipeline.rs:766` `real_binary_build_and_dev_cover_client_bundling_contract` | `env-gate` (esbuild) | **CI**: `health.yml` → "Run zfb client-bundling cross-pipeline acceptance test". Local: `ZFB_ESBUILD_BIN=<abs path> cargo test -p zfb --test client_bundling_cross_pipeline -- --ignored`. Level-4 real `zfb build` + `zfb dev --port 0`; serialized by both the flock and nextest `e2e-heavy` group. |
