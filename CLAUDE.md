...
- **`cargo-nextest` (ADOPTED — issue #1340):** CI runs the Rust test suite under `cargo nextest run --workspace --profile ci` (health.yml), configured in `.config/nextest.toml`. This gives the CI retry budget, per-test timeouts, and JUnit output — the mechanisms below are now real, not aspirational.
  ...
  - **Heavy e2e binaries are serialized by a nextest `test-group`.** nextest schedules test *binaries* concurrently by default, which would boot several real `zfb dev`/`zfb build` V8+esbuild processes at once. `.config/nextest.toml` defines a `[test-groups.e2e-heavy]` with `max-threads = 1` and an override assigning **14** heavy binaries to it: the **7 dev-server** binaries (`build_terminates`, `dev_bind_before_walk_e2e`, `dev_build_static_parity`, `dev_dep_invalidation_1284_e2e`, `dev_public_large_tree_smoke_e2e`, `dev_serve_e2e`, `dev_serve_injected_routes_e2e`) — the same set #1339's cross-binary flock covers — plus **7 build-command** binaries (`build_cleans_outdir`, `build_package_routes`, `build_package_routes_consumer`, `client_router_autoinclude_build`, `content_snapshot_no_deferred`, `css_modules_components_build`, `end_to_end_basic_blog_build`) that each spawn `zfb build` (booting V8 + esbuild) but are NOT in #1339's flock. For the dev-server 7 the flock and test-group are **defense in depth** (flock = OS-level guarantee inside the test bodies; test-group = runner-level guarantee); for the build-command 7 the test-group is the **sole** serialization guard. (`check_command` and `framework_packages_no_pnpm` are deliberately excluded — the former runs only `zfb check`, the latter calls `zfb_build::bundle` in-process without spawning `zfb` or booting V8.) That group also carries a long per-test `slow-timeout` (`terminate-after` = 600s) so the legitimate ~280s dev-server watchdog tests are never killed as "hung"; everything else gets a tighter 300s kill. The same override (issue #1363) ALSO joins one individual heavy test by name — `render_pipeline::tests::eval_deferred_paths_via_worker_embedded_v8_non_literal_paths` — via a `test(=...)` predicate rather than `binary(=...)`, since it lives inside the `zfb` crate's shared lib-test binary (not its own integration-test binary) alongside hundreds of ordinary unit tests; this keeps it serialized against the other 14 binaries when exam.yml's weekly `quarantine-heavy` job runs it. A SECOND `test(=...)` predicate joins the same override (issue #1394): `version_stamp_from_env` (tagged `heavy`, see the manifest below) clocked in at 134s against this group's 300s kill budget when exam.yml's `quarantine-heavy` job ran it, and belongs in the serialized group for the same CPU/memory-contention reason as the rest — it IS its own binary (unlike the render_pipeline case), but `test(=...)` was used for consistency with that entry.
...
Audited and retagged in full during issue #1337 (2026-07); reconciled again during issue #1363 (2026-07) after the #1352/#1353/#1354 pending-feature tests were activated or retagged. Table below lists every `#[ignore]`d test as of that reconciliation and its scheduled home — kept accurate for #1344/#1349 to consume. Update this table whenever a `#[ignore]` is added, removed, or reclassified.
...
| `crates/zfb-islands/tests/integration.rs:622` `islands_shadow_raw_import_bundles_text` | `env-gate` (esbuild) | Same as above. |
| `crates/zfb-islands/tests/integration.rs:683` `client_script_raw_import_bundles_text` | `env-gate` (esbuild) | Same as above. |
| `crates/zfb-islands/tests/integration.rs:748` `island_module_worker_emits_contract_companion_and_dev_layout` | `env-gate` (esbuild) | Same as above. |
| `crates/zfb-islands/tests/integration.rs:908` `island_css_import_bundles_without_error` | `env-gate` (esbuild) | Same as above. |
| `crates/zfb-islands/tests/integration.rs:970` `island_module_css_import_bundles_without_error` | `env-gate` (esbuild) | Same as above. |
| `crates/zfb-islands/tests/integration.rs:1242` `islands_shadow_expands_glob_and_executes` | `env-gate` (esbuild) | Same as above. |
| `crates/zfb-islands/tests/integration.rs:1313` `islands_shadow_preserve_symlinks_is_load_bearing` | `env-gate` (esbuild) | Same as above. |
...
- Heavy opt-in: `B4PUSH_FULL=1 pnpm b4push` additionally runs the **full health.yml parity set** (issue #1332) — not just the workspace test suite: `cargo nextest run --workspace` (or `cargo test --workspace` when nextest is absent) using nextest's **default** profile (`retries = 0`, unlike CI's `--profile ci`); `cargo test --workspace --doc` (nextest branch only — nextest doesn't run doctests); the `zfb-md-extras` `test-utils`-gated suite (`cargo nextest run -p zfb-md-extras --features test-utils`) plus its scoped `cargo clippy`; `cargo check --no-default-features -p zfb --tests` (the V8-off build); the `zfb-islands` esbuild env-gate suite (`cargo test -p zfb-islands --tests -- --ignored`); and the 3 tailwindcss-v4 env-gate tests (`zfb-css`, `zfb-build`, `zfb --lib commands::build::`) that `health.yml` also runs for the first time as of issue #1393. Before #1332 the md-extras/no-v8 lanes had no local parity; the islands/tailwindcss env-gate lanes were the same gap, closed by #1393 — a green `B4PUSH_FULL=1` run did not imply `health.yml` would pass without them. The 4 env-gate steps are skipped (not failed) if their binary slot was never staged locally (run `cargo build --workspace --all-targets` first).
