...
Audited and retagged in full during issue #1337 (2026-07); reconciled again during issue #1504 (2026-07). The table below lists all **31** ignored Rust tests (**25** `env-gate`, **5** `heavy`, **1** `verification`) and their scheduled homes. Update this table whenever a `#[ignore]` is added, removed, or reclassified.
...
| `crates/zfb/tests/wasm_ssr_adapter_e2e.rs:375` `wrangler_dry_run_attaches_runtime_wasm_as_compiled_module` | `env-gate` (wrangler 4.85.0) | Local: `ZFB_ESBUILD_BIN=<abs path> cargo test -p zfb --test wasm_ssr_adapter_e2e -- --ignored`. Level-4 real `zfb build` plus pinned `wrangler deploy --dry-run --no-bundle --outdir`, using Wrangler's default `CompiledWasm` rule; it confirms the exact copied SSR-only Wasm import reaches the module table as `compiled-wasm` with matching emitted bytes, and skips when Wrangler is unavailable. Serialized by nextest `e2e-heavy`. |
