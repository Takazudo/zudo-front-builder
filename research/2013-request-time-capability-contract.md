...
`globalThis.__zfb.limits` is **frozen** (`Object.freeze`, including the `allowedFetchSchemes`
array): the JS-side caps are defence in depth, and bundle code that could *raise* one would wave an
oversized payload past the JS pre-check into the op. The polyfill also reads the object through a
closure captured at boot rather than off `globalThis.__zfb`, since the bridge *global* is writable
(the emitted bundle prelude assigns to it) even though the object is not. Tests that need a
different cap boot the host with one — `EmbeddedV8RenderHost::with_limits_override` — instead of
mutating it from JS. Every JS-side cap is still enforced independently in Rust, which is the
...
| **Redirects** | Only **301, 302, 303, 307, 308** are redirect statuses `[spec]` — every other 3xx (notably 300 and 304) is an ordinary response and is returned to the caller unchanged in all three redirect modes. `redirect: "follow"` (default), `"manual"`, `"error"` `[spec]`. `follow` chases up to `MAX_REDIRECTS`; `manual` returns the redirect response unchanged with `redirected = false`; `error` makes a redirect status a network error. Method rewriting per `[spec]`: **303** on any method other than `GET`/`HEAD` becomes `GET` with the body dropped; **301/302** on `POST` becomes `GET` with the body dropped; 301/302 on any other method, and 307/308 on every method, preserve method and body. When the method is rewritten to `GET`, `content-length`, `content-type`, `content-encoding`, and `content-language` are dropped `[spec]`. Cross-origin redirects additionally strip `authorization`, `cookie`, and `proxy-authorization` `[spec]`. | `MAX_REDIRECTS = 20` `[spec]` — the Fetch standard redirect count; Cloudflare documents no divergence | `TypeError` — `fetch(<url>): too many redirects (limit 20)`; for `redirect:"error"`, `TypeError: fetch(<url>): redirect not allowed (redirect mode is "error")` | denied | supported | Each hop counts against `MAX_SUBREQUESTS_PER_DISPATCH`, matching Cloudflare, where every hop in a redirect chain is a subrequest `[wd]`. |
...
| **Abort** | `init.signal` and `Request.signal` are honoured. `AbortController` / `AbortSignal` / `AbortSignal.timeout()` / `AbortSignal.abort()` must be **added** to `web_polyfills.js` — they do not exist today `[zfb]`. An already-aborted signal rejects synchronously without opening a socket. `signal.reason` is used as the rejection value when set, else an `AbortError` `[spec]`. | n/a | `Error` with `name = "AbortError"` — `The operation was aborted.` (a real `DOMException` is unavailable — see divergences) | denied | supported | Aborting **drops the Rust future**, which cancels the in-flight `reqwest` request and closes the socket. The op registers a `CancelHandle` under a per-call `cancelId` on its first (eager) poll; the JS abort listener calls `op_zfb_fetch_cancel(cancelId)` before it rejects the caller's promise, so the transport is dropped rather than left running to the deadline. The subrequest slot already claimed stays spent — a hop that reached the wire is not un-made. Cancellation mid-body is covered by `an_abort_mid_body_cancels_the_transport_and_closes_the_socket` (`embedded_v8/js_fetch_tests.rs`), which holds the host alive while asserting the server saw the connection close. |
...
| Feature | Supported behavior | Limits | Error type / message | SSG behavior | SSR behavior | Cancellation / resource limits |
| --- | --- | --- | --- | --- | --- | --- |
| **Dispatch mode** | A **per-dispatch** flag, not a per-host one. `crates/zfb/src/ssr_adapter.rs` drives request-time SSR through the *same* `EmbeddedV8RenderHost` that the dev tick uses to prerender SSG pages, so a host-level flag would either deny legitimate SSR or open build-time network access. New `DispatchMode { BuildTime, RequestTime }` on `HttpRequestLike` (`crates/zfb-render/src/embedded_v8/dispatch.rs`), defaulting to `BuildTime`; threaded through `dispatch_fetch` into `__zfb.dispatch(url, method, headers, body, mode)` in `embedded_v8/js/globals_shim.js`, which publishes it as `__zfb.mode` for the duration of the dispatch. `web_polyfills.js` reads it through a closure the shim installs at boot, **not** off `globalThis.__zfb` — the bridge global is writable, so re-reading it let bundle code substitute `{ mode: "request-time" }` and take the request-time branch during a build-time render. **The mode also reaches Rust**, as `DispatchModeState` in the host's `OpState`, and that is the enforcement that counts: see [Where the mode is enforced](#where-the-mode-is-enforced). | one flag, two values | n/a | `BuildTime` | `RequestTime` | The mode belongs to the dispatch that set it. Host-authorised dispatches are tracked as a stack: when one settles, the innermost still-running dispatch's mode is restored, and when none is left **that dispatch's own mode stands** rather than reverting — otherwise anything a request-time handler started without awaiting would report itself as an SSG denial at request time. An UNAUTHORISED (bundle-originated) `dispatch` call neither selects nor restores the mode, so a floating one cannot republish a stale value. Nothing is widened by the residual: every dispatch sets the mode unconditionally, and the host resets it to `BuildTime` before evaluating any module. |
...
| **Who sets `RequestTime`** | Exactly one production call site: `crates/zfb/src/ssr_adapter.rs`. **`EmbeddedV8Host::dispatch_fetch_full` is NOT a usable seam on its own** — `crates/zfb/src/commands/dev.rs:4688` calls the very same method for the build-time dev content-trace endpoint, so putting `RequestTime` inside the `ThreadedV8Host` override would enable networking for that caller too, while leaving the default would deny SSR. The `EmbeddedV8Host` trait therefore gains a **mode-bearing** entry point — either a `dispatch_fetch_full_with_mode(..., mode: DispatchMode)` method whose existing `dispatch_fetch_full` delegates to it with `BuildTime`, or an added `mode` parameter with every current call site updated to `BuildTime` explicitly. Either shape is acceptable; what is **not** acceptable is inferring mode from the endpoint, the URL, or the presence of a body. `ssr_adapter.rs` is the only caller that passes `RequestTime`. Everything else — `zfb-build`'s SSG page renderer, `crates/zfb/src/render_pipeline.rs` prerendering, the dev content-trace call, `paths()` evaluation, `ThreadedConfigEvaluator` — stays `BuildTime`. Production Workers routes never go through this host at all; workerd runs the bundle directly. | n/a | n/a | all other call sites | one call site | n/a |

### Where the mode is enforced

`globalThis.Deno.core.ops.op_zfb_fetch` is reachable from bundle code — the polyfill is one caller
of it, not a gate in front of it — so a build-time denial that lives only in `web_polyfills.js` is
a denial the bundle can walk around. **`op_zfb_fetch` is the trust boundary.** It reads
`DispatchModeState` out of `OpState`, where no JS value can point, and rejects anything that is not
`RequestTime` with the byte-identical policy message the polyfill uses, before the transport is
entered at all. The JS-side reader, the nonce, and the frozen limits are defence in depth for the
*diagnostic*; the *policy* does not depend on any of them.

The `OpState` cell is installed per dispatch and deliberately **not** cleared when the dispatch
settles — a `fetch` a handler started and never awaited belongs to the dispatch that started it.
It is reset to `BuildTime` before every module evaluation, which is the only other place bundle
code runs.

The mode nonce that authorises *choosing* a mode from JS is 256 bits from the OS CSPRNG
(`getrandom::fill`, the same source `crypto.rs` uses), compared in constant time. Its previous
construction — process id, wall-clock nanoseconds, and a counter — was reconstructible rather than
unguessable.
...
`op_zfb_fetch_cancel` is synchronous, and this is not a violation either: it takes no I/O path at
all — it looks up a `CancelHandle` by id and flags it — and an abort must take effect on the turn
it happens rather than after another event-loop round trip.
