...
/// project root may still be feeding one to esbuild (that half of the contract
/// is pinned by the unit tests in `src/esbuild.rs`).
#[test]
#[ignore = "env-gate: esbuild binary — ZFB_ESBUILD_BIN=<absolute path to pinned esbuild> \
            cargo test -p zfb-islands --test integration \
            working_dir_is_clean_after_multi_entry_bundle -- --ignored"]
fn working_dir_is_clean_after_multi_entry_bundle() {
    let project = tempfile::tempdir().expect("tempdir");
    let root = project.path();
    stage_minimal_node_modules(root);
    std::fs::create_dir_all(root.join("components")).unwrap();

    let alpha = root.join("components/Alpha.tsx");
    let beta = root.join("components/Beta.tsx");
    std::fs::write(&alpha, "export default function Alpha() { return null; }\n").unwrap();
    std::fs::write(&beta, "export default function Beta() { return null; }\n").unwrap();

    // A previous `zfb dev` / `zfb build` killed mid-bundle: its NamedTempFile
    // Drop never ran, so the entry is still sitting in the project root.
    let stranded = root.join(".zfb-esbuild-entry-plnMVZ.tsx");
    std::fs::write(&stranded, "// Generated by zfb-islands\n").unwrap();
    let aged = std::fs::OpenOptions::new()
        .write(true)
        .open(&stranded)
        .unwrap();
    aged.set_modified(std::time::SystemTime::now() - std::time::Duration::from_secs(24 * 60 * 60))
        .unwrap();
    ...
    let leftovers: Vec<String> = std::fs::read_dir(root)
        ...
        .filter(|name| name.starts_with(".zfb-esbuild-entry-"))
        .collect();
    assert!(
        leftovers.is_empty(),
        "a successful multi-entry bundle must leave no synthesized entry files \
