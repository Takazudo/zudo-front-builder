...
//!    found under `pages/`, plus the hydration shim, and re-exports a
//!    `routes` map of route-path → page module + a `hydrateIsland`
//!    function. This is the single load-bearing module miniflare and
//!    the runtime SSR adapter consume.
...
//! with two named exports the runtime contract pins:
//!
...
pub fn bundle(input: BundlerInput) -> Result<BundlerOutput> {
    ...
    write_entry_module(shadow, &routes).context("bundler: failed writing entry.mjs")?;
...
/// Generate the `entry.mjs` module that re-exports `routes` and
/// `hydrateIsland`. This is the single load-bearing module miniflare
/// (T6) and the runtime SSR adapter (T2) consume.
fn write_entry_module(shadow: &Path, routes: &[RouteEntry]) -> Result<()> {
    use std::fmt::Write as _;
    let mut src = String::new();
    src.push_str("// AUTO-GENERATED by zfb_build::bundler. Do not edit.\n");
    src.push_str("// Single ESM entry shared by miniflare (T6) and the runtime SSR adapter.\n");
    src.push_str("// Exports: { routes, hydrateIsland }.\n\n");
    src.push_str(&format!(
