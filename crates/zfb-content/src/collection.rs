    ...
    T: DeserializeOwned + garde::Validate<Context = ()>,
{
    ...
    for path in files {
        ...
        if let Some(p) = pipeline.as_deref_mut() {
            p.reset_per_entry();
            // Per-file source context for ResolveLinksPlugin (no-op
            // when the plugin isn't wired; the file path also arms the
            // zfb#1030 URL-space fallback for non-index pages). The
            // bundler does the same in
