...
| `ZFB_DEV_BOOT_LAZY=1` | Serve a valid prebuilt `dist/` immediately and render each route on first request; warns and falls back to the eager boot render when no servable `dist/` exists |
| `ZFB_DEV_BOOT_LAZY=cold` | Seedless variant of the above: render each route on first request without requiring a prebuilt `dist/` at all — a route with no fallback artifact on disk serves the dev 404 page (with livereload) until its own first request; a leftover `dist/` from an unrelated prior build still serves its (possibly stale) bytes first, same as Auto's seed |
