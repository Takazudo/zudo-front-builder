...
/// - **Runtime / non-literal** (`NonLiteral`, e.g. `getCollection(...)`):
///   import the package's real `paths` under an alias and re-declare a
///   top-level wrapper `export async function paths() { return
///   __zfb_pkg_paths(...); }`. The extractor sees a top-level `paths` whose
///   body is a CALL → `NonLiteral` → deferred to `eval_deferred_paths_via_worker`,
///   which runs the **bundled** module's real `paths()` in V8 via
///   `GET /__paths__/<route_key>`. Because the overlay module IS what the
///   bundler bundles, the imported `__zfb_pkg_paths` resolves to the real
///   package `paths` inside the worker.
...
pub(crate) fn synthesize_dynamic_overlay_module(
    ...
    let extraction =
        zfb_render::paths_extract::extract_paths(&source, &file_name).map_err(|e| {
            anyhow!(
                "parse error extracting `paths` from {}: {e}",
                entrypoint.display()
            )
        })?;
    ...
    match extraction {
        ...
        zfb_render::paths_extract::PathsExtraction::NonLiteral { .. } => {
            // Runtime paths(): import the package's real `paths` and wrap
            // it top-level. The extractor sees a CALL body → NonLiteral →
            // deferred to the V8 `__paths__` worker, which runs THIS module
            // (and thus the imported real `paths`) inside the bundle.
            out.push_str(
