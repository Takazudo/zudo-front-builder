...
//!    owns the embedded V8 host session. The asset pipeline's
...
//!    one HTTP round-trip — not a fresh host boot.
//!
...
//! dev loop would orphan the host process.
//!
//! ## Configuration
//!
...
fn boot_dev_renderer(
    ...
    let state = start(RendererStartInput {
        ...
        // The embedded V8 host URL is wired in by Sub 6 (ADR-007).
        // For now supply an empty base URL as a placeholder.
