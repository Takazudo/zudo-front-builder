  ...
  for (const p of plugins) {
    ...
    const ctx = {
      ...
      injectRoute(pattern, entrypoint, opts) {
        ...
        // pages/ grammar.
        //
        // A preset may register "/" in both `zfb dev` and `zfb build`. The
        // downstream route resolver gives a matching user `pages/` route
        // precedence, including `pages/index` over an injected root. Without
        // a user index, the injected root is staged and rendered in either
        // mode. Dev request dispatch also leaves devMiddleware ahead of the
        // injected fallback for user-owned routes.
        if (pattern.includes("//")) {
          throw new Error(
